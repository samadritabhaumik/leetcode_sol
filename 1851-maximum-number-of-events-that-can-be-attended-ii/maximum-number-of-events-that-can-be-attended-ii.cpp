class Solution {
    ...
    int n;
    int attend(vector<vector<int>>& events,int pos,int k){
        if(k==0 || pos>=n)
            return 0;
        if(mem[pos][k]!=-1)
            return mem[pos][k];  
        int skip_event = attend(events,pos+1,k);
        int next = next_event[pos];
        int attend_event = attend(events,next,k-1) + events[pos][2];
